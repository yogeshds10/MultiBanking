 ...
 * @author client
 */
@WebServlet(name = "DepositHandler", urlPatterns = {"/DepositHandler"})
public class DepositHandler extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps.setInt(1, Integer.parseInt(request.getParameter("idaccount")));
            ps.setFloat(2, 0);
            ps.setFloat(3, Float.parseFloat(request.getParameter("amount")));
            ps.setFloat(4, total);
            ps.executeUpdate();
            request.setAttribute("msg", "Amount Credited Successfully.");
            request.getRequestDispatcher("home.jsp").forward(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(DepositHandler.class.getName()).log(Level.SEVERE, null, ex);
            request.setAttribute("msg", "Unable to Credit Amount. Try Again Later");
